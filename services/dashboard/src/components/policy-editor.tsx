...
export function PolicyEditor({ policy, canManage = true }: { policy: Policy; canManage?: boolean }) {
  ...
  return (
    <form action={formAction} className="space-y-4">
      <div className="space-y-1">
        ...
        <Input id="name" name="name" defaultValue={policy.name} disabled={ro} />
      </div>
      <div className="space-y-1">
        <Label htmlFor="agents">Agents (comma-separated; empty = all agents)</Label>
        <Input id="agents" name="agents" defaultValue={policy.agents.join(", ")} placeholder="procurement-agent-v2" disabled={ro} />
      </div>
      <div className="space-y-1">
        <Label htmlFor="rules">
        ...
        <Textarea id="rules" name="rules" rows={14} defaultValue={JSON.stringify(policy.rules, null, 2)} disabled={ro} />
      </div>
      <div className="flex items-center gap-3">
        {canManage ? (
          ...
          <span className="text-sm text-[var(--muted)]">Read-only — an admin role is required to edit policies.</span>
        )}
